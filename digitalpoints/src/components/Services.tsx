...
const services = [
  { title: "Digital Marketing", description: "We help businesses reach the right audience through social media, content, paid campaigns, and practical digital strategies designed to increase visibility, engagement, and enquiries.", tags: ["Social Media", "Content Marketing", "Paid Campaigns", "Digital Strategy"], visual: "MARKETING" },
  { title: "Video Production", description: "We create engaging video content that helps businesses tell their story, showcase their work, explain their products, and connect with customers across digital platforms.", tags: ["Corporate Video", "Social Content", "Event Coverage", "Video Editing"], visual: "MEDIA" },
  { title: "Printing", description: "We turn your designs into professional printed materials that keep your business visible, consistent, and credible wherever your customers meet your brand offline.", tags: ["Business Cards", "Flyers", "Posters", "Large Format"], visual: "PRINT" },
  { title: "Promotional Items", description: "We help businesses stay memorable with useful branded items made for campaigns, events, teams, customers, and everyday brand visibility.", tags: ["Branded Gifts", "Corporate Items", "Campaign Materials", "Custom Orders"], visual: "PROMOTE" },
  { title: "Branding Services", description: "We build visual identities that make businesses easier to recognise, trust, and remember, from the first impression to everyday brand communication.", tags: ["Logo Design", "Brand Identity", "Brand Guidelines", "Creative Direction"], visual: "BRAND" },
  { title: "Embroidery", description: "We add a professional branded finish to clothing and fabric products with clean, durable embroidery made for teams, businesses, schools, and organisations.", tags: ["Workwear", "Uniforms", "Caps", "Custom Embroidery"], visual: "EMBROIDERY" },
] as const;
  ...
  return (
    <motion.article aria-label={service.title} style={{ x, opacity, scale, rotate, zIndex: services.length - index }} className="absolute inset-0 overflow-hidden rounded-[26px] bg-[#08bdb8] shadow-[0_30px_90px_rgba(0,0,0,0.16)] sm:rounded-[32px] lg:rounded-[38px]">
      <div className="relative flex h-full flex-col p-6 sm:p-9 lg:p-12 xl:p-14">
        ...
        <div className="relative z-10 flex items-center justify-between gap-5"><div className="flex items-center gap-3 text-[#145b59] sm:gap-4"><span className="h-px w-10 bg-[#145b59]/70 sm:w-16" /><span className="font-poppins text-[10px] font-medium uppercase tracking-[0.28em] sm:text-xs sm:tracking-[0.32em]">Our Services</span></div><span className="hidden font-poppins text-[10px] font-medium uppercase tracking-[0.28em] text-black/30 sm:block">{service.visual}</span></div>
        <div className="relative z-10 mt-7 max-w-5xl sm:mt-10 lg:mt-12"><h3 className="font-display text-[clamp(2.5rem,7vw,6.7rem)] font-semibold leading-[0.88] tracking-[-0.065em] text-black">{service.title}</h3><p className="mt-5 max-w-4xl font-display text-[clamp(1rem,1.75vw,1.48rem)] leading-[1.42] tracking-[-0.02em] text-[#101818] sm:mt-7">{service.description}</p></div>
        <div className="relative z-10 mt-6 flex flex-wrap gap-2 sm:mt-8">{service.tags.map((tag) => <span key={tag} className="border border-black/10 bg-black/[0.045] px-3 py-2 font-poppins text-[10px] font-medium text-black sm:px-3.5 sm:text-xs">{tag}</span>)}</div>
        <div className="relative z-10 mt-auto grid grid-cols-2 items-end gap-5 pt-7 sm:grid-cols-[1fr_auto] sm:pt-10"><div className="hidden max-w-md sm:block"><div className="h-px w-full bg-black/15" /><p className="mt-3 font-poppins text-[10px] uppercase tracking-[0.18em] text-black/40">Creative solutions built around your business</p></div><span className="inline-flex w-fit bg-[#211f1f] px-5 py-3 font-poppins text-xs font-semibold text-white sm:px-6 sm:py-3.5 sm:text-sm">Learn More</span></div>
      </div>
    </motion.article>
  );
}
